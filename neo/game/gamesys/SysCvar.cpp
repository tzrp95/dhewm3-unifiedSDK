...
idCVar g_fov(						"g_fov",					"90",				CVAR_GAME | CVAR_ARCHIVE | CVAR_INTEGER | CVAR_NOCHEAT, "sets the field of view" );
...
idCVar af_physics(					"af_physics",						"1",		CVAR_GAME | CVAR_NOCHEAT | CVAR_BOOL,	"Enable or disabe physics for ragdolls" );
...
// HDR (Denton)
idCVar r_useHDR(					"r_useHDR",							"1",		CVAR_GAME | CVAR_ARCHIVE | CVAR_BOOL, "Enables HDR Rendering & post-processing." );
idCVar r_hdrPostProcess(			"r_hdrPostProcess",					"1",		CVAR_GAME | CVAR_ARCHIVE | CVAR_INTEGER, "Activates HDR bloom . \n 1: HDR Bloom with automatic eye exposure" );
idCVar r_hdrMiddleGray(				"r_hdrMiddleGray",					"0.1",		CVAR_GAME | CVAR_ARCHIVE | CVAR_FLOAT,	"Middle gray value for overall scene . Range 0 - 1. (Works only when automatic exposure is on)" );
idCVar r_hdrBrightPassThreshold(	"r_hdrBrightPassThreshold",			"1.7",		CVAR_GAME | CVAR_ARCHIVE | CVAR_FLOAT, "brightness threshold for Bright-pass (Works only when automatic exposure is on)" );
idCVar r_hdrBrightPassOffset(		"r_hdrBrightPassOffset",			"5.0",		CVAR_GAME | CVAR_ARCHIVE | CVAR_FLOAT,	"Brightness offset for bright pass (Works only when automatic exposure is on)" );
idCVar r_hdrMinLuminance(			"r_hdrMinLuminance",				"0.003",	CVAR_GAME | CVAR_ARCHIVE | CVAR_FLOAT,	"Luminance is restricted to this value to control whiteness when in pitch dark areas. (Works only when automatic exposure is on)" );
idCVar r_hdrMaxLuminance(			"r_hdrMaxLuminance",				"0.22",		CVAR_GAME | CVAR_ARCHIVE | CVAR_FLOAT,	"Luminance is restricted to this value to control darkness when in super-bright areas. (Works only when automatic exposure is on)" );
idCVar r_hdrEyeAdjustmentDelay(		"r_hdrEyeAdjustmentDelay",			"2.2",		CVAR_GAME | CVAR_ARCHIVE | CVAR_FLOAT,	"Total time in second taken to adjust eye exposure.(Works only when automatic exposure is on)" );
idCVar r_hdrEyeAdjustmentBias(		"r_hdrEyeAdjustmentBias",			"0.09",		CVAR_GAME | CVAR_ARCHIVE | CVAR_FLOAT, "Automatic eye exposure factor (range 0 to 1) \n e.g. 0.0 - No automatic eye exposure, \n 0.5 - 50% automatic eye exposure, \n 1.0 Full automatic eye exposure." );
idCVar r_hdrEyeAdjustmentBloomBias( "r_hdrEyeAdjustmentBloomBias",		"1",		CVAR_GAME | CVAR_ARCHIVE | CVAR_FLOAT,	"Bloom can have a separate eye adjustment bias. See cvar descritption for r_hdrEyeAdjustmentBias." );
idCVar r_hdrColorCurveBias(			"r_hdrColorCurveBias",				"0.26",		CVAR_GAME | CVAR_ARCHIVE | CVAR_FLOAT,	"Applies Exponential Color Curve to final pass (range 0 to 1), 1 = color curve fully applied , 0= No color curve" );
idCVar r_hdrSceneExposure(			"r_hdrSceneExposure",				"1.0",		CVAR_GAME | CVAR_ARCHIVE | CVAR_FLOAT,	"Scene exposure. Try values lower or greater than 1.0" );
idCVar r_hdrGammaCorrection(		"r_hdrGammaCorrection",				"1.0",		CVAR_GAME | CVAR_ARCHIVE | CVAR_FLOAT,	"Gamma Correction." );
idCVar r_hdrMaxColorIntensity(		"r_hdrMaxColorIntensity",			"1.0",		CVAR_GAME | CVAR_FLOAT,	"Adjusting this value will cause color burnout in a controllable fashion (range 0.0 - 256.0) \nHigher values reduces the contrast, lower values increases the contrast." );
idCVar r_hdrBloomIntensity(			"r_hdrBloomIntensity",				"0.6",		CVAR_GAME | CVAR_ARCHIVE | CVAR_FLOAT,	"Adjusts the Bloom intensity. 0.0 disables both bloom as well as halo passes" );
idCVar r_hdrHaloIntensity(			"r_hdrHaloIntensity",				"1.0",		CVAR_GAME | CVAR_ARCHIVE | CVAR_FLOAT,	"Adjusts the Halo intensity. Set to 0.0 disables halo pass." );
idCVar r_hdrVignetteBias(			"r_hdrVignetteBias",				"0.45",		CVAR_GAME | CVAR_ARCHIVE | CVAR_FLOAT,	"Vignette: Fading-of-edges-of-the-screen effect(range 0 to 1)" );												
idCVar r_hdrLumUpdateRate(			"r_hdrLumUpdateRate",				"11",		CVAR_GAME | CVAR_ARCHIVE | CVAR_INTEGER, "Updates luminance data after every these many number of frames." );												
idCVar r_hdrEnableDebugMode(		"r_hdrEnableDebugMode",				"0",		CVAR_GAME | CVAR_INTEGER, "Shows all the textures generated for HDR postprocess. \n 1: Shows all textures \n 2: Decodes and shows all textures" );
idCVar r_hdrDebugTextureIndex(		"r_hdrDebugTextureIndex",			"0",		CVAR_GAME | CVAR_ARCHIVE | CVAR_INTEGER, "Show intermediate textures used for HDR postprocess \n 1: scene Image(64x Scaled down) \n 2: Average Luminance Initial pass texture\n 3: Average luminance" );
...
idCVar r_useAmbientLight(			"r_useAmbientLight",				"0",						CVAR_GAME | CVAR_NOCHEAT | CVAR_ARCHIVE | CVAR_BOOL,	"Enable a global ambient light bound to player" );
idCVar r_ambientLightRadius(		"r_ambientLightRadius",				"1024 1024 1024",			CVAR_GAME | CVAR_NOCHEAT | CVAR_ARCHIVE,	"Sets the ambient light's radius (XYZ = 0 to 65536)" );
idCVar r_ambientLightColor(			"r_ambientLightColor",				"0.03125 0.03125 0.03125",	CVAR_GAME | CVAR_NOCHEAT | CVAR_ARCHIVE,	"Sets the ambient light's color (RGB = 0.0 to 1.0)" );
...
idCVar g_testStuff(					"g_testStuff",						"0",		CVAR_BOOL, "Does nothing, it's here just for testing various stuff" );
idCVar g_weaponDepthHack(			"g_weaponDepthHack",				"1",		CVAR_BOOL, "Crunch z depth on weapons" );
idCVar g_muzzleFlashShadows(		"g_muzzleFlashShadows",				"0",		CVAR_BOOL | CVAR_ARCHIVE, "Cast shadows from weapons muzzleflashes" );
...
idCVar g_gibVelocity(				"g_gibVelocity",					"1",		CVAR_GAME | CVAR_INTEGER | CVAR_ARCHIVE, "Gibs velocity" );
idCVar g_gibLifeTime(				"g_gibLifeTime",					"0",		CVAR_GAME | CVAR_FLOAT | CVAR_ARCHIVE, "Gibs lifetime\n <= 0 is default" );

idCVar g_weaponSprint(				"g_weaponSprint",					"0",		CVAR_GAME | CVAR_NOCHEAT | CVAR_ARCHIVE | CVAR_BOOL, "Toggles weapon sprinting");
idCVar g_weaponAwareness(			"g_weaponAwareness",				"0",		CVAR_GAME | CVAR_NOCHEAT | CVAR_ARCHIVE | CVAR_BOOL, "Toggles weapon awareness near walls or npcs" );

// Player Related
idCVar pm_thirdPersonOffset(		"pm_thirdPersonOffset",				"0",		CVAR_GAME | CVAR_NETWORKSYNC | CVAR_ARCHIVE | CVAR_FLOAT, "Camera offset from player in 3rd person (-n = left, +n = right)" ); // (Sikkpin)
idCVar pm_awarenessDistance(		"pm_awarenessDistance",				"54",		CVAR_GAME | CVAR_NOCHEAT | CVAR_ARCHIVE | CVAR_FLOAT, "Distance of the awareness system" );

idCVar g_playerOverride(			"g_playerOverride",					"0",		CVAR_BOOL | CVAR_ARCHIVE, "Overrides the automatic player selection" );
idCVar g_player(					"g_player",							"0",		CVAR_GAME | CVAR_ARCHIVE | CVAR_INTEGER, "Changes the player, requires map restart\n"
																												"0 = player_default ( not yet added )\n"
																												"1 = player_doommarine\n"
																												"2 = player_doommarine_d3xp\n"
																												"3 = player_doommarine_d3lm\n");
...
idCVar pm_abilityModifierActive(	"pm_ability_active",				"0",		CVAR_GAME | CVAR_ARCHIVE | CVAR_INTEGER, "Changes the active ability modifier\n"
																												"0 = None\n"
																												"1 = Damage Scaler (take less danage) \n"
...
idCVar pm_energy(					"pm_energy",						"100",		CVAR_GAME | CVAR_NETWORKSYNC | CVAR_FLOAT, "Amount of energy to use with ability modifiers" );
idCVar pm_energyrate(				"pm_energyrate",					"25",		CVAR_GAME | CVAR_NETWORKSYNC | CVAR_FLOAT, "Rate that player regains energy. divide pm_energy by this value to determine how long it takes to fully recharge." );

// Health / Armor Regeneration -->
idCVar pm_healthRegenTime(			"pm_healthRegenTime",				"1",		CVAR_GAME | CVAR_NOCHEAT | CVAR_ARCHIVE | CVAR_INTEGER, "How often to regenerate health" );
idCVar pm_healthRegenDelay(			"pm_healthRegenDelay",				"5",		CVAR_GAME | CVAR_NOCHEAT | CVAR_ARCHIVE | CVAR_INTEGER, "Delay (in seconds) before health starts to regenerate after taking damage" );
idCVar pm_healthRegenAmt(			"pm_healthRegenAmt",				"1",		CVAR_GAME | CVAR_NOCHEAT | CVAR_ARCHIVE | CVAR_INTEGER, "How much health to regenerate per pm_healthRegenTime" );
idCVar pm_healthRegenLimit(			"pm_healthRegenLimit",				"100",		CVAR_GAME | CVAR_NOCHEAT | CVAR_ARCHIVE | CVAR_INTEGER, "Total amount of health that can be regenerated" );
idCVar pm_healthRegenStepsNormal(	"pm_healthRegenStepsNormal",		"1",		CVAR_GAME | CVAR_NOCHEAT | CVAR_ARCHIVE | CVAR_INTEGER, "Splits pm_healthRegenLimit into n number of steps, regenerating only that portion, only for g_skill < 2 ( like Chronicles of Riddick / WolfTNO )" );
idCVar pm_healthRegenStepsHard(		"pm_healthRegenStepsHard",			"4",		CVAR_GAME | CVAR_NOCHEAT | CVAR_ARCHIVE | CVAR_INTEGER, "Splits pm_healthRegenLimit into n number of steps, regenerating only that portion, only for g_skill => 2 ( like Chronicles of Riddick / WolfTNO )" );

idCVar pm_armorRegenReq(			"pm_armorRegenReq",					"1",		CVAR_GAME | CVAR_NOCHEAT | CVAR_ARCHIVE | CVAR_INTEGER, "How much armor the player needs to have before regenerating can take place" );
idCVar pm_armorRegenTime(			"pm_armorRegenTime",				"2",		CVAR_GAME | CVAR_NOCHEAT | CVAR_ARCHIVE | CVAR_INTEGER, "How often to regenerate armor" );
idCVar pm_armorRegenDelay(			"pm_armorRegenDelay",				"5",		CVAR_GAME | CVAR_NOCHEAT | CVAR_ARCHIVE | CVAR_INTEGER, "Delay (in seconds) before armor starts to regenerate after taking damage" );
idCVar pm_armorRegenAmt(			"pm_armorRegenAmt",					"1",		CVAR_GAME | CVAR_NOCHEAT | CVAR_ARCHIVE | CVAR_INTEGER, "How much armor to regenerate per pm_armorRegenTime" );
idCVar pm_armorRegenLimit(			"pm_armorRegenLimit",				"125",		CVAR_GAME | CVAR_NOCHEAT | CVAR_ARCHIVE | CVAR_INTEGER, "Total amount of armor that can be regenerated" );
idCVar pm_armorRegenStepsNormal(	"pm_armorRegenStepsNormal",			"1",		CVAR_GAME | CVAR_NOCHEAT | CVAR_ARCHIVE | CVAR_INTEGER, "Splits pm_armorRegenLimit into n number of steps, regenerating only that portion, only for g_skill < 2 ( like Chronicles of Riddick / WolfTNO )" );
idCVar pm_armorRegenStepsHard(		"pm_armorRegenStepsHard",			"5",		CVAR_GAME | CVAR_NOCHEAT | CVAR_ARCHIVE | CVAR_INTEGER, "Splits pm_armorRegenLimit into n number of steps, regenerating only that portion, only for g_skill = 2 ( like Chronicles of Riddick / WolfTNO )" );
idCVar pm_armorRegenSoundTime(		"pm_armorRegenSoundTime",			"5",		CVAR_GAME | CVAR_NOCHEAT | CVAR_ARCHIVE | CVAR_INTEGER, "How often to play the armor patch sound when regenerating armor" );

idCVar pm_doubleJumpPower(			"pm_doubleJumpPower",				"3.5",		CVAR_GAME | CVAR_NOCHEAT | CVAR_ARCHIVE | CVAR_FLOAT,	"Double jump power" );

idCVar pm_godTime(					"pm_godTime",						"20",		CVAR_GAME | CVAR_NOCHEAT | CVAR_ARCHIVE | CVAR_FLOAT,	"How much god ability lasts. Divide pm_energy to this number to see how long it lasts." );
idCVar pm_godRechargeRate(			"pm_godRechargeRate",				"25",		CVAR_GAME | CVAR_NOCHEAT | CVAR_ARCHIVE | CVAR_FLOAT,	"Energy recharge rate when god ability is used" );

idCVar pm_strengthTime(				"pm_strengthTime",					"10",		CVAR_GAME | CVAR_NOCHEAT | CVAR_ARCHIVE | CVAR_FLOAT,	"How much super strength lasts. Divide pm_energy to this number to see how long it lasts." );
idCVar pm_strengthRechargeRate(		"pm_strengthRechargeRate",			"25",		CVAR_GAME | CVAR_NOCHEAT | CVAR_ARCHIVE | CVAR_FLOAT,	"Energy recharge rate when god ability is used" );
idCVar pm_strengthMultiplier(		"pm_strengthMultiplier",			"10",		CVAR_GAME | CVAR_NOCHEAT | CVAR_ARCHIVE | CVAR_FLOAT,	"Your melee attacks do this time more damage when strength ability is on" );

idCVar pm_speedTime(				"pm_speedTime",						"10",		CVAR_GAME | CVAR_NOCHEAT | CVAR_ARCHIVE | CVAR_FLOAT,	"How much super strength lasts. Divide pm_energy to this number to see how long it lasts." );
idCVar pm_speedRechargeRate(		"pm_speedRechargeRate",				"25",		CVAR_GAME | CVAR_NOCHEAT | CVAR_ARCHIVE | CVAR_FLOAT,	"Energy recharge rate when god ability is used" );
idCVar pm_speedMultiplier(			"pm_speedMultiplier",				"2.5",		CVAR_GAME | CVAR_NOCHEAT | CVAR_ARCHIVE | CVAR_FLOAT,	"Multiply player's speed by this number when speed ability." );


idCVar pm_invisTime(				"pm_invisTime",						"20000",	CVAR_GAME | CVAR_NOCHEAT | CVAR_ARCHIVE | CVAR_FLOAT,	"How much invisibility ability lasts. Time in msec." );
idCVar pm_invisRechargeRate(		"pm_invisRechargeRate",				"25",		CVAR_GAME | CVAR_NOCHEAT | CVAR_ARCHIVE | CVAR_FLOAT,	"Energy recharge rate when god ability is used" );
idCVar pm_invisDrainTime(			"pm_invisDrainTime",				"5",		CVAR_GAME | CVAR_NOCHEAT | CVAR_ARCHIVE | CVAR_FLOAT,	"Divide pm_energy to this number to get the result. Time should match the pm_invisTime." );

idCVar pm_longJumpVelocityZ(		"pm_longJumpVelocityZ",				"300",		CVAR_GAME | CVAR_NOCHEAT | CVAR_ARCHIVE | CVAR_FLOAT,	"Long jump velocity" );
idCVar pm_longJumpVelocity(			"pm_longJumpVelocity",				"1.7",		CVAR_GAME | CVAR_NOCHEAT | CVAR_ARCHIVE | CVAR_FLOAT,	"Long jump velocity on Z axis" );
idCVar pm_longJumpRechargeRate(		"pm_longJumpRechargeTime",			"75",		CVAR_GAME | CVAR_NETWORKSYNC | CVAR_FLOAT, "Energy recharge rate when longjump is used" );

idCVar pm_maxDashes(				"pm_maxDashes",						"2",		CVAR_GAME |CVAR_INTEGER,  "Max times the player can dash before having to recharge" );
idCVar pm_dashDistance(				"pm_dashDistance",					"5000",		CVAR_GAME |CVAR_INTEGER,  "Distance of the dash" );
idCVar pm_dashRechargeRate(			"pm_dashRechargeRate",				"25",		CVAR_GAME | CVAR_NETWORKSYNC | CVAR_FLOAT, "Energy recharge rate when dashing is used" );
