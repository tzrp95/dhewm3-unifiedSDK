...
void idAASLocal::DeleteClusterCache( int clusterNum ) {
	...
	for ( int i = 0; i < file->GetCluster( clusterNum ).numReachableAreas; i++ ) {
		for ( idRoutingCache *cache = areaCacheIndex[clusterNum][i]; cache; cache = areaCacheIndex[clusterNum][i] ) {
...
void idAASLocal::DeletePortalCache( void ) {
	...
	for ( int i = 0; i < file->GetNumAreas(); i++ ) {
		for ( idRoutingCache *cache = portalCacheIndex[i]; cache; cache = portalCacheIndex[i] ) {
...
aasHandle_t idAASLocal::AddObstacle( const idBounds &bounds ) {
	...
	idRoutingObstacle *obstacle = new idRoutingObstacle;
...
void idAASLocal::DeleteOldestCache( void ) const {
	...
	idRoutingCache *cache = cacheListStart;
	UnlinkCache( cache );
...
idReachability *idAASLocal::GetAreaReachability( int areaNum, int reachabilityNum ) const {
	for ( idReachability *reach = file->GetArea( areaNum ).reach; reach; reach = reach->next ) {
